...
router.post('/updatePatientUser', function(req, res, next) {
  ...
  if(err){
   ...
   con.query("update appointmentplus.user  set password='"+ password+"', address='"+ address+"',phone='"+ phone+"',city='"+ city+"',province='"+ province+"',postalCode='"+ postalCode +"' where email='"+email+"'", function (err, result, fields) {
    if(err){
